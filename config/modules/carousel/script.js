...
(function() {
    function updateSlideNumbers() {
        const container = document.getElementById('carousel-rows-container');
        if (!container) return;
    ...
    function initCarouselModule() {
        ...
        const addBtn = document.getElementById('add-slide-row-btn');
        if (addBtn) {
            const newBtn = addBtn.cloneNode(true);
            addBtn.parentNode.replaceChild(newBtn, addBtn);
            
            newBtn.addEventListener('click', function() {
                const template = document.getElementById('tmpl-carousel-slide');
                if (!template) return;
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initCarouselModule);
    } else {
        initCarouselModule();
    }
})();