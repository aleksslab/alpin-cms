<?php
/**
 * Модуль: Слайдер продвинутый — админка
 */
...
<div class="space-y-4">
    ...
    <div class="bg-slate-50/50 p-4 rounded-xl border border-slate-200 space-y-3">
        ...
        <div class="editor-row">
            <div class="editor-field">
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Эффект перехода</label>
                <select name="effect" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-[var(--primary-color)]">
                    <?php foreach ($effectOptions as $key => $label): ?>
                        <option value="<?php echo $key; ?>" <?php echo $effect == $key ? 'selected' : ''; ?>><?php echo $label; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="editor-field">
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Скорость (мс)</label>
                <input type="number" name="speed" value="<?php echo $speed; ?>" min="200" max="2000" step="100" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-[var(--primary-color)]">
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-2 pt-2">
            <label class="flex items-center gap-2 cursor-pointer text-sm text-slate-600">
                ...
                <input type="checkbox" name="autoplay" value="1" <?php echo $autoplay ? 'checked' : ''; ?> class="w-4 h-4 text-[var(--primary-color)] rounded border-slate-300 focus:ring-[var(--primary-color)]">
                Автопрокрутка
            </label>
            <label class="flex items-center gap-2 cursor-pointer text-sm text-slate-600">
                <input type="hidden" name="pause_on_hover" value="0">
                <input type="checkbox" name="pause_on_hover" value="1" <?php echo $pauseOnHover ? 'checked' : ''; ?> class="w-4 h-4 text-[var(--primary-color)] rounded border-slate-300 focus:ring-[var(--primary-color)]">
                Остановка при наведении
            </label>
            <label class="flex items-center gap-2 cursor-pointer text-sm text-slate-600">
                <input type="hidden" name="swipe" value="0">
                <input type="checkbox" name="swipe" value="1" <?php echo $swipe ? 'checked' : ''; ?> class="w-4 h-4 text-[var(--primary-color)] rounded border-slate-300 focus:ring-[var(--primary-color)]">
                Swipe на мобилках
            </label>
            <label class="flex items-center gap-2 cursor-pointer text-sm text-slate-600">
                <input type="hidden" name="show_arrows" value="0">
                <input type="checkbox" name="show_arrows" value="1" <?php echo $showArrows ? 'checked' : ''; ?> class="w-4 h-4 text-[var(--primary-color)] rounded border-slate-300 focus:ring-[var(--primary-color)]">
                Показывать стрелки
            </label>
            <label class="flex items-center gap-2 cursor-pointer text-sm text-slate-600">
                <input type="hidden" name="show_dots" value="0">
                <input type="checkbox" name="show_dots" value="1" <?php echo $showDots ? 'checked' : ''; ?> class="w-4 h-4 text-[var(--primary-color)] rounded border-slate-300 focus:ring-[var(--primary-color)]">
                Показывать точки
            </label>
        </div>
    </div>
    ...
    <!-- БЛОК 2: Слайды -->
    <div id="slider-rows-container" class="space-y-3">
        <?php 
        $slideTemplate = '
    ...
    <button type="button" id="add-slide-btn" class="w-full py-2.5 border border-dashed border-slate-300 text-slate-500 font-medium rounded-lg hover:border-[var(--primary-color)] hover:text-[var(--primary-color)] transition-all cursor-pointer text-sm flex items-center justify-center gap-2">
